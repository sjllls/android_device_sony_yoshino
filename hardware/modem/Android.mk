...
CUSTOMIZATION_OVERLAY_SYMLINK := $(PRODUCT_OUT)/vendor
$(CUSTOMIZATION_OVERLAY_SYMLINK): $(LOCAL_INSTALLED_MODULE)
	$(hide) ln -sf $(OEM_OVERLAY_DIR) $@
ALL_DEFAULT_INSTALLED_MODULES += \
	$(CUSTOMIZATION_OVERLAY_SYMLINK)
...
include $(CLEAR_VARS)
LOCAL_MODULE := init.sony.oem-overlay.sh
LOCAL_SRC_FILES := init.sony.oem-overlay.sh
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE_STEM := init.sony.oem-overlay.sh
LOCAL_MODULE_CLASS := EXECUTABLES
LOCAL_MODULE_PATH := $(TARGET_OUT_VENDOR_EXECUTABLES)
include $(BUILD_PREBUILT)
